...
try {
    switch ($method) {
        case 'POST':
            ...
            $announcementId = $_POST['lectureId'] ?? null; // Mantendo nome do form
            ...
            // Upload da imagem (com caminho para document root)
            $imagePath = '';
            ...
            if (empty($announcementId) || strpos($announcementId, 'default-') === 0) {
                ...
                $stmt = $pdo->prepare("
                    ...
                    ON DUPLICATE KEY UPDATE title=VALUES(title), speaker=VALUES(speaker), announcement_date=VALUES(announcement_date), lecture_time=VALUES(lecture_time), description=VALUES(description), image_path=IF(? <> '', VALUES(image_path), image_path)
                ");
                $stmt->execute([$newId, $title, $speaker, $announcementDate, $lectureTime, $description, $imagePath, $imagePath]);
        ...
        case 'DELETE':
            // ... (Lógica de deleção mantida, garantindo que o DELETE funcione sem o campo lecture_time)
            if (isset($_GET['id'])) {
                $announcementId = $_GET['id'];
                
                // 1. Buscar caminho da imagem para deletar
                $stmt = $pdo->prepare("SELECT image_path FROM upcoming_announcements WHERE id = ?");
                $stmt->execute([$announcementId]);
                $announcement = $stmt->fetch(PDO::FETCH_ASSOC);
                
                // 2. Deletar imagem do servidor se existir
                ...
                // 3. Deletar anúncio do banco de dados
