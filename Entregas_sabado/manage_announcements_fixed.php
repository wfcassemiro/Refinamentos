...
try {
    switch ($method) {
        case 'POST':
            ...
            $lectureTime = $_POST['lectureTime'] ?? '19:00';
            ...
            if (empty($announcementId) || strpos($announcementId, 'default-') === 0) {
                ...
                // Verificar se a coluna lecture_time existe
                try {
                    $stmt = $pdo->prepare("
                    ...
                    // Se a coluna lecture_time não existir, inserir sem ela
                    if (strpos($e->getMessage(), 'lecture_time') !== false) {
                        $stmt = $pdo->prepare("
                            INSERT INTO upcoming_announcements (id, title, speaker, announcement_date, description, image_path, display_order) 
                            VALUES (?, ?, ?, ?, ?, ?, (SELECT IFNULL(MAX(display_order), 0) + 1 FROM upcoming_announcements AS ua))
                        ");
                        $stmt->execute([$newId, $title, $speaker, $announcementDate, $description, $imagePath]);
                    } else {
                        throw $e;
                    }
                }
                ...
                try {
                    ...
                    // Se a coluna lecture_time não existir, atualizar sem ela
                    if (strpos($e->getMessage(), 'lecture_time') !== false) {
                        if (!empty($imagePath)) {
                            $stmt = $pdo->prepare("
                                UPDATE upcoming_announcements 
                                SET title = ?, speaker = ?, announcement_date = ?, description = ?, image_path = ?, updated_at = NOW() 
                                WHERE id = ?
                            ");
                            $stmt->execute([$title, $speaker, $announcementDate, $description, $imagePath, $announcementId]);
                        } else {
                            $stmt = $pdo->prepare("
                                UPDATE upcoming_announcements 
                                SET title = ?, speaker = ?, announcement_date = ?, description = ?, updated_at = NOW() 
                                WHERE id = ?
                            ");
                            $stmt->execute([$title, $speaker, $announcementDate, $description, $announcementId]);
                        }
                    } else {
                        throw $e;
                    }
        ...
        case 'GET':
            if (isset($_GET['id'])) {
                ...
                try {
                    ...
                    // Se a coluna lecture_time não existir, buscar sem ela
                    if (strpos($e->getMessage(), 'lecture_time') !== false) {
                        $stmt = $pdo->prepare("SELECT id, title, speaker, announcement_date, description, image_path FROM upcoming_announcements WHERE id = ?");
                        $stmt->execute([$_GET['id']]);
                    } else {
                        throw $e;
                    }
                ...
                try {
                    ...
                    // Se a coluna lecture_time ou is_active não existir
                    $stmt = $pdo->query("
                        SELECT id, title, speaker, announcement_date, description, image_path
                        FROM upcoming_announcements 
                        ORDER BY announcement_date ASC
                        LIMIT 3
                    ");
