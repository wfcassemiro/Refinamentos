...
try {
    switch ($method) {
        case 'POST':
            ...
            // Nota: lecture_time não existe na tabela, então não vamos usá-lo
            ...
            if (empty($announcementId) || strpos($announcementId, 'default-') === 0) {
                // Inserir novo anúncio (sem lecture_time)
                ...
                // Atualizar anúncio existente (sem lecture_time)
                if (!empty($imagePath)) {
                    $stmt = $pdo->prepare("
        ...
        case 'GET':
            if (isset($_GET['id'])) {
                // Buscar anúncio específico (sem lecture_time)
                ...
                if ($announcement) {
                    ...
                    // Adicionar lecture_time padrão já que o frontend espera
                    $announcement['lecture_time'] = '19:00'; 
