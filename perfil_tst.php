...
// NOVA FUNCIONALIDADE: Buscar lista de palestras que o usuário quer assistir (watchlist)
$user_watchlist = [];
try {
    $query = "
    ...
    // Debug temporário
    error_log("Query watchlist perfil para user_id: $user_id");
    
    $stmt = $pdo->prepare($query);
    $stmt->execute([$user_id]);
    $user_watchlist = $stmt->fetchAll();
    
    // Debug: contar itens encontrados
    error_log("Watchlist items encontrados: " . count($user_watchlist));
} catch (Exception $e) {
    // Debug temporário - remover em produção
    error_log("ERRO na watchlist do perfil: " . $e->getMessage());
    
    // Verificar se a tabela existe
    try {
        $check_table = $pdo->query("SHOW TABLES LIKE 'user_watchlist'");
        $table_exists = $check_table->fetch() ? 'SIM' : 'NÃO';
        error_log("Tabela user_watchlist existe: $table_exists");
    } catch (Exception $e2) {
        error_log("Erro ao verificar tabela: " . $e2->getMessage());
    }
    
    $user_watchlist = [];
}

// Buscar lista de certificados/palestras do usuário
$user_certificates = [];
try {
    $stmt = $pdo->prepare("
        SELECT c.*, 
...
<div class="main-content">
    ...
    <div class="video-card profile-card" style="background: #fff3cd; border: 1px solid #ffeaa7;">
        <h3>🔍 Debug da Watchlist</h3>
        <p><strong>User ID:</strong> <?php echo $user_id; ?></p>
        <p><strong>Itens na watchlist:</strong> <?php echo count($user_watchlist); ?></p>
        <p><strong>Dados:</strong></p>
        <pre style="font-size: 0.8em; max-height: 200px; overflow-y: auto;"><?php print_r($user_watchlist); ?></pre>
    </div>
    <?php endif; ?>
                ...
                <div class="watchlist-info">
                    ...
                    <div class="watchlist-actions" onclick="event.stopPropagation();">
                        <div class="watchlist-buttons">
                            <a href="/palestra.php?id=<?php echo $item['lecture_id']; ?>" class="btn-watch">
...
<style>
/* Estilos para a nova seção de watchlist no perfil */
.watchlist-grid {
    display: grid;
